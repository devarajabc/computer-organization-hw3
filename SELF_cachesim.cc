...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  size_t idx = (addr >> idx_shift) & (sets-1);
  size_t way=0;
  size_t min_ll=time[idx][0]*count[idx][0]+time[idx][0]+count[idx][0];
  for(int i=0;i<ways;i++)
  {
    if(time[idx][i]*count[idx][i]+time[idx][i]+count[idx][i]<min_ll)
    {
      min_ll=time[idx][i]*count[idx][i]+time[idx][i]+count[idx][i];
      way=i;
    }
  }
  count[idx][way]=0;
